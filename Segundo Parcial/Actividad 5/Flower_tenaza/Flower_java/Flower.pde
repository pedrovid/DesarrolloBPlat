class Flower {
  ...
  public void update() {
    ...
    if(mouseX>=x-centerSize/2&&mouseX<=x-centerSize/2+centerSize&&mouseY>=y-centerSize/2&&mouseY<=y-centerSize/2+centerSize)
      arduino.servoWrite(4, constrain(int((x-centerSize/2)+centerSize-mouseX), 20, 160));
    ellipseMode(CENTER);
    fill(centerColor);
    noStroke();
